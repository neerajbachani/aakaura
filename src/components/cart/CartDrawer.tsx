...
export function CartDrawer({ isOpen, onClose }: CartDrawerProps) {
  ...
  const { data: suggestions } = useQuery({
    queryKey: ["cart-suggestions", cartCategories],
    queryFn: async () => {
      // Fetch up to 12 products to have enough pool to filter from
  ...
  const recommendedProducts = useMemo(() => {
    ...
    if (cartCategories.length > 0) {
      const categoryMatches = availableSuggestions.filter((p) =>
        cartCategories.includes(p.categoryId),
      );
      const others = availableSuggestions.filter(
        (p) => !cartCategories.includes(p.categoryId),
      );

      // Merge: category matches first, then fill remainder with others if we need more
      availableSuggestions = [...categoryMatches, ...others];
    }
                    ...
                    <div className="flex h-full flex-col bg-[#191919] border-l border-[#BD9958]/20 shadow-2xl">
                      ...
                      >
                        {isLoading || authLoading ? (
                          ...
                          <div className="flex flex-col items-center justify-center h-64 text-[#BD9958]/70 px-6">
                            ...
                            <h3 className="font-cormorant text-2xl mb-2 text-[#BD9958]">
                              Please log in
                            </h3>
                          ...
                          <div className="px-6 py-6">
                            ...
                            {/* Suggested Products matching current categories */}
                            {recommendedProducts &&
                              recommendedProducts.length > 0 && (
                                <div className="mt-10 pt-8 border-t border-[#BD9958]/20">
                                  <h3 className="font-cormorant text-xl text-[#BD9958] mb-4 uppercase tracking-widest italic font-light">
                                    You may also like
                                  </h3>
                      ...
                      {cart && cart.items.length > 0 && (
                        <div className="border-t border-[#BD9958]/20 px-6 py-6 bg-[#27190b]/50 backdrop-blur-sm">
                          <CartSummary
                            cart={cart}
                            className="!bg-transparent !border-none !p-0 !shadow-none mb-6"
                          />

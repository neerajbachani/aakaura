...
export default function ChakraJourneyTemplate({
  ...
  const currentProducts = React.useMemo(() => {
    ...
    return propProducts.map((p, i) => {
      ...
      const staticP = staticProducts.find(sp => sp.id === p.id) || staticProducts[i];
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-0 left-0 right-0 p-6 md:p-8">
        ...
        >
          <span className="max-w-sm block mx-auto md:mx-0">AAKAURA'S {chakra.tone.toUpperCase()} COLLECTION</span>
        </h2>
      </div>
            ...
            <div className="w-full md:max-w-[300px] flex justify-center md:justify-start items-center gap-4 order-2 md:order-1">
                {product.variants && product.variants.length > 0 ? (
                    <div className="flex items-center gap-3 bg-black/20 backdrop-blur-md px-4 py-2 md:px-6 md:py-3 rounded-full border border-white/10">
                        ...
                        <span className="ml-2 text-xs opacity-80 tracking-widest whitespace-nowrap">{activeVariant?.name}</span>
                    </div>
                ) : (
                    <span className="max-w-sm text-center md:text-left hidden md:block">{product.name.toUpperCase()}</span>
                )}
            </div>

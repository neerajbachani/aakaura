...
async function main() {
  try {
    console.log('📋 Updating bonsai products - separating specs from premium detailing...\n');
    ...
    for (const journey of journeys) {
      ...
      for (const clientType of ['soul-luxury', 'energy-curious']) {
        ...
        products.forEach((product: any, index: number) => {
          if (product.id && product.id.includes('bonsai')) {
            // Update designBreakdown (premium detailing) - NO specifications here
            products[index].designBreakdown = bonsaiPremiumDetailing;
            
            // Update specifications object with ALL required fields
            products[index].specifications = {
              'Product Name': product.name,
              'Product Type': 'Chakra-Inspired Living Bonsai',
              'Plant Type': 'Adenium (Desert Rose)',
              'Chakra': product.specifications?.Chakra || (product.chakra ? product.chakra : 'N/A'),
              'Color Palette': product.specifications?.['Color Palette'] || (product.colors ? product.colors : 'N/A'),
            ...
            // Set care instructions as formatted list (will appear in Specifications section)
            products[index].careInstructions = careInstructionsList;

            // Ensure idealFor is set
      ...
      if (updated) {
        ...
        console.log(`  💾 Saved updates to ${journey.slug}`);
      }
    }

    console.log(`\n🎉 Successfully updated ${totalUpdates} bonsai products!`);
    console.log('\n✨ Changes made:');
    console.log('   • Premium detailing: Only narrative content (no specifications)');
    console.log('   • Specifications section: All product specs including care');
    console.log('   • Care instructions: Listed once, formatted as bullets');
    console.log('   • No more duplication between sections\n');

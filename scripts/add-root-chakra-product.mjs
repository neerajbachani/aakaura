/**
 * Script to add Root Chakra Wall Hanging product to the database
 * Usage: node scripts/add-root-chakra-product.mjs
 */

const API_URL = 'http://localhost:3000';
const JOURNEY_SLUG = 'grounding';

// Root Chakra Wall Hanging Product Data
...
async function addProduct(token, clientType, product) {
    console.log(`\n📦 Adding ${clientType} product: ${product.name}...`);

    const response = await fetch(`${API_URL}/api/admin/journeys/${JOURNEY_SLUG}/products`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
    ...
    if (!response.ok) {
        console.error(`❌ Failed to add ${clientType} product:`, data.error);
        return false;
    }

    console.log(`✅ Successfully added ${clientType} product!`);
    return true;
}

async function main() {
    try {
        console.log('🚀 Starting product addition script...\n');
        ...
        const sl_success = await addProduct(token, 'soul-luxury', soulLuxuryProduct);

        // Add Energy-Curious product
        const ec_success = await addProduct(token, 'energy-curious', energyCuriousProduct);

        console.log('\n' + '='.repeat(50));
        console.log('📊 Summary:');
        console.log(`Soul-Luxury: ${sl_success ? '✅ Added' : '❌ Failed'}`);
        console.log(`Energy-Curious: ${ec_success ? '✅ Added' : '❌ Failed'}`);
        console.log('='.repeat(50));

        if (sl_success && ec_success) {
            console.log('\n🎉 All products added successfully!');
            console.log(`\n🌐 View the products at: ${API_URL}/journey/grounding`);
        } else {
